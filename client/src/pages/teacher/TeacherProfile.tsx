  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      setProfile({
        ...
        avatar: data.avatar || '',
        employeeId: (data as any).teacherProfile?.employeeId || '',
        designation: (data as any).teacherProfile?.designation || '',
        qualification: (data as any).teacherProfile?.qualification || '',
      })
      ...
      setProfile((prev) =>
        prev
          ? {
              ...
              qualification: (updated as any).teacherProfile?.qualification || prev.qualification,
            }
          : prev
      )
      setUser(updated)
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-800 mt-0.5">
                  Your email and password setup is complete. Please update your profile photo, contact details, and info below to finalize onboarding.
                </p>
              </div>
            </div>
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate('/teacher/dashboard', { replace: true })}
              className="shrink-0"
            >
      ...
      <div className="rounded-xl border border-border bg-surface p-6 sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 min-w-0">
            ...
            {profile.employeeId && (
              <p className="mt-1 text-xs text-text-muted">Employee ID: {profile.employeeId}</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 border-t border-border pt-6">
          <h3 className="text-lg font-semibold text-text mb-4">Professional Information</h3>
          <div className="grid gap-6 sm:grid-cols-2">
            <Field
              label="Designation"
              icon={<BriefcaseIcon className="h-4 w-4 text-primary" />}
              value={profile.designation || 'Not provided'}
              editable={false}
              hint="Designation is managed by the administrator."
            />
            <Field
              label="Qualification"
              icon={<svg className="h-4 w-4 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c0 2 6 3 6 3s6-1 6-3v-5"/></svg>}
              value={profile.qualification || 'Not provided'}
              editable={isEditing}
              onChange={(value) => setProfile((prev) => prev ? { ...prev, qualification: value } : prev)}
            />
          </div>
        </div>
...
function Field({
  ...
  type?: 'select' | 'date' | 'number' | 'textarea'
